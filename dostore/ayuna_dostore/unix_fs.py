import asyncio
from io import BytesIO
from typing import Any, AsyncGenerator, List

import ayuna_core.fileops as flops
from ayuna_core.basetypes import AyunaError, ErrorOrBytesIO, ErrorOrStr

from .base import BaseDoStore, BulkPutEntry, UnixFsDoStoreConfig


class UnixFsStore(BaseDoStore):
    """Unix File System DoStore implementation."""

    def __init__(
        self, *, config: UnixFsDoStoreConfig, aio_loop: asyncio.AbstractEventLoop
    ):
        super().__init__(config, aio_loop)
        self._config = config

        if self._config.is_readonly:
            if not flops.is_dir_readable(self._config.sroot):
                raise AyunaError(
                    f"Store's root folder is not readable: {self._config.sroot}"
                )
        elif not flops.is_dir_writable(self._config.sroot, check_creatable=True):
            raise AyunaError(
                f"Store's root folder is not writable: {self._config.sroot}"
            )

    def object_store_path(self, key: str):
        if key.startswith(f"{self._config.sroot}/"):
            return key

        if key.startswith("/"):
            return f"{self._config.sroot}{key}"

        return f"{self._config.sroot}/{key}"

    async def close(self) -> None:
        await asyncio.sleep(0)
        self._logger.info(f"Closing {self._config.typid} store: {self._store_id}")
        await super().close()

    async def get_object(self, key: str) -> ErrorOrBytesIO:
        try:
            opath = self.object_store_path(key=key)
            odata = await self.add_thread_pool_task(flops.read_bytes, opath)

            if not isinstance(odata, bytes):
                raise TypeError(f"Expected bytes from read_bytes, got {type(odata)}")

            return BytesIO(odata)
        except Exception as ex:
            error_mesg = f"Failed to read object: {key} from {self._config.typid} store: {self._store_id}"

            self._logger.error(error_mesg, exc_info=ex)
            return AyunaError(error_mesg, exc_cause=ex)

    async def put_object(
        self, key: str, data: BytesIO, overwrite: bool = False
    ) -> ErrorOrStr:
        if self._config.is_readonly:
            return AyunaError(
                f"Store: {self._store_id} is read-only, cannot write object: {key}"
            )

        try:
            opath = self.object_store_path(key=key)
            result = await self.add_thread_pool_task(
                flops.write_bytes, opath, data.getvalue(), overwrite=overwrite
            )

            if not isinstance(result, int):
                raise TypeError(f"Expected int from write_bytes, got {type(result)}")

            if result == -1:
                return AyunaError(
                    f"Object: {key} already exists in store: {self._store_id}"
                )

            return f"Written {result} bytes for object: {key}"
        except Exception as ex:
            error_mesg = f"Failed to write object: {key} to {self._config.typid} store: {self._store_id}"

            self._logger.error(error_mesg, exc_info=ex)
            return AyunaError(error_mesg, exc_cause=ex)

    async def delete_object(self, key: str) -> ErrorOrStr:
        if self._config.is_readonly:
            return AyunaError(
                f"Store: {self._store_id} is read-only, cannot delete object: {key}"
            )

        try:
            opath = self.object_store_path(key=key)
            await self.add_thread_pool_task(flops.delete_file, opath)

            return f"Deleted object: {key}"
        except Exception as ex:
            error_mesg = f"Failed to delete object: {key} from {self._config.typid} store: {self._store_id}"

            self._logger.error(error_mesg, exc_info=ex)
            return AyunaError(error_mesg, exc_cause=ex)

    async def get_objects(self, keys: List[str]) -> AsyncGenerator[ErrorOrBytesIO, Any]:
        if not keys:
            yield AyunaError("Keys list is empty")
            return

        for okey in keys:
            result = await self.get_object(key=okey)
            yield result

    async def put_objects(
        self, entries: List[BulkPutEntry]
    ) -> AsyncGenerator[ErrorOrStr, Any]:
        if not entries:
            yield AyunaError("Entries list is empty")
            return

        if self._config.is_readonly:
            yield AyunaError(
                f"Store: {self._store_id} is read-only, cannot write objects"
            )
            return

        for okey, data, overwrite in entries:
            result = await self.put_object(key=okey, data=data, overwrite=overwrite)
            yield result

    async def delete_objects(self, keys: List[str]) -> AsyncGenerator[ErrorOrStr, Any]:
        if not keys:
            yield AyunaError("Keys list is empty")
            return

        if self._config.is_readonly:
            yield AyunaError(
                f"Store: {self._store_id} is read-only, cannot delete objects"
            )
            return

        for okey in keys:
            result = await self.delete_object(key=okey)
            yield result

    async def object_exists(self, key: str) -> bool:
        await asyncio.sleep(0)
        return flops.file_exists(self.object_store_path(key))

    async def total_objects(self) -> int:
        await asyncio.sleep(0)
        return flops.total_files_in_dir(base_dir=self._config.sroot)
